...
YUV2RGB = np.array([[1.164,  0.000,  1.596],
                    [1.164, -0.392, -0.813],
                    [1.164,  2.017,  0.000]])
...
while True:
    ...
    frame_yuv = np.dstack((frame_y, frame_u, frame_v)).astype(np.float32)
    frame_rgb = np.dot(frame_yuv, YUV2RGB.T).clip(0, 255).astype(np.uint8)
